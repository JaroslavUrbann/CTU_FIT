...
// mode, low, high
int getData(char* mode, int* base, long long int* low, long long int* high){
	int status = scanf(" %c%d%lld%lld", mode, base, low, high);
	
	if (status == EOF)
		return 0;

	if (status == 4 && (*mode == 'l' || *mode == 'c') && *low <= *high && *low >= 0 && *base >= 2 && *base <= 36)
		return 1;
...
void printPalindrome(int base, ll num, int max_power){
	...
	while (max_power >= 0){
		int number = num / (int)pow(base, max_power) % base;
		char c;
		if (number <= 9)
			c = number + '0';
		else
...
int main(){
	...
	int status;
	while (1){
		status = getData(&mode, &base, &low, &high);
		if (status == -1)
			WRONG_INPUT
		if (status == 1)
